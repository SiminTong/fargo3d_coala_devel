...
### Data
path    = "../../outputs/fargo_multifluid_size/"
nx      = 384
ny      = 256
n       = 1
...
im1 = ax1.imshow(log10(rhod1/rhog), origin='lower', aspect='auto', cmap=cmap,vmax=log10(rhod1/rhog).max(),vmin=log10(rhod1/rhog).min())
cb1  = colorbar(im1, cax=ax2,orientation="horizontal")
cb1.ax.set_xlabel(r"$\log_{10}(\rho_{\rm d1}/\rho_{\rm g})$", fontsize=14)

im2 = ax3.imshow(log10(rhod2/rhog), origin='lower', aspect='auto', cmap=cmap, vmax=log10(rhod2/rhog).max(),vmin=log10(rhod2/rhog).min())
...
#tight_layout()
savefig("develop_plotdata_size.png",  bbox_inches='tight', dpi=250)
